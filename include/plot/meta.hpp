 ...
 *
 * Compile-time argument dispatch for the dependency-free SVG plotting layer:
 * order-independent named arguments (`plot::arg::tpos`/`get`/`getn`/`required`),
 * compile-time tuple iteration (`plot::impl::static_for`), and the detection
 * idiom (`plot::impl::is_detected`). This is a self-contained copy — the plot
 * layer carries its own machinery so it compiles with ZERO sibling libraries on
 * the include path. Pure C++ — no third-party or platform dependency.
 */
